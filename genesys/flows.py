...
class InboundShortMessage:
    ...
    def class_asdict(self) -> dict:
        ...
        if self.states is not None:
            for state in self.states:
                aux_state = {"state": OrderedDict({})}
                for chave, valor in asdict(state).items():
                    if valor is not None:
                        aux_state["state"][chave] = valor
                if aux_state:
                    tasks.append(aux_state)

        if states:
            data["inboundShortMessage"]["states"] = states
        else:
            if data["inboundShortMessage"].get("states"):
                del data["inboundShortMessage"]["states"]
        return data